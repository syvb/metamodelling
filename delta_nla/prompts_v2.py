...
1. CAUSAL EFFECT ON THE EVENTUAL OUTPUT: how the eventual output differs with and without this change ("gains" rose because of it, "loses" fell). This is the primary basis for your explanation. Its labels are clues to a topic, entity, relation or judgement being worked on; paraphrase a label into a concept only when its referent is clear from the text, otherwise omit it. Never combine labels into a relation that neither the text nor the lists support. It is split into the part that came from gathering earlier text and the part recalled in place.
2. EARLIER TEXT DRAWN ON: earlier positions whose content fed the change (offset = how many tokens back; share = fraction). Mention an earlier passage only when it explains what gained or lost weight. A high sink_frac with small shares means the change came mostly from internal recall, not from the text; do not imply retrieval then.
...
- Sentence 3: what is drawn in from earlier text, or consolidated from internal recall (say which).
...
Rules:
- Use impersonal active phrasing ("Attention narrows to...", "Emphasis moves from... to..."); never refer to the model, the network or "it" as an agent thinking in the third person.
